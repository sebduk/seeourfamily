...
<%
dim lngIDPhoto, lngIDPersonne, intPos, intCpt, strError
...
if Request("todo") = "update" then
	...
	if request("del") <> empty then
		...
		if Request("NomPhoto") <> rs0("NomPhoto") and Request("NomPhoto") <> empty then
			...
			strItemOld = server.mappath(strDocument & rs0("NomPhoto"))
			strItemNew = server.mappath(strDocument & Request("NomPhoto"))
		...
		if Request("Date") <> empty then
			rs0("Date") = Request("Date")
		else
			rs0("Date") = null
		end if
...
if Request("DocDetails") <> empty then
	Dim strTN

	strTN = FindTN(strImage, Request("DocDetails"))
	Response.Write "<form action=docPage.asp method=post>" 
	Response.Write "<input type=hidden name=todo value=add>" 
	Response.Write "<input type=hidden name=NomPhoto value=""" & Request("DocDetails") & """>" 

	Response.Write "<table border=0 cellpadding=0 cellspacing=0>"
	Response.Write "<tr><td align=right>"

	Response.Write "File Name&nbsp;<input type=text name=Nothing value=""" & Request("DocDetails") & """ size=30 class=box><br>"
	Response.Write "Date&nbsp;<input type=text name=Date size=30 class=box><br>"
	...
	Response.Write "<a href=""" & strDocument & Request("DocDetails") & """ target=_blank>"
...
elseif lngIDPhoto <> empty then
	...
	if not rs1.eof then
		...
		Response.Write "<form action=docPage.asp name=myForm method=post>" 
		...
		Response.Write "Date <input type=text size=30 name=Date value=""" & rs1("Date") & """ class=box><br>"
		Response.Write "<textarea name=DescrPhoto cols=30 rows=15 class=box>"
		...
		Response.Write "<a href=""" & strDocument & rs1("NomPhoto") & """ target=_blank>"
...
function FindTN(MyFolder, MyFileName)
	select case LCase(right(MyFileName, 3))
		...
		case "doc"
			FindTN = "/Image/Icon/doc.gif"
		case "mdb"
			FindTN = "/Image/Icon/mdb.gif"
		case "pdf"
			FindTN = "/Image/Icon/pdf.gif"
		case "ppt"
			FindTN = "/Image/Icon/ppt.gif"
		case "pps"
			FindTN = "/Image/Icon/pps.gif"
		case "txt"
			FindTN = "/Image/Icon/txt.gif"
		case "xls"
			FindTN = "/Image/Icon/xls.gif"
		case "zip"
			FindTN = "/Image/Icon/zip.gif"
		case else
			FindTN = "/Image/Icon/other.gif"
	end select
