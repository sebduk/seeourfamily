...
if Request("todo") = "update" then
	...
	if Request("Del") <> empty then
		...
		if Request("Link") <> empty then
			strRequestLink = Request("Link")
			strRequestLink = Replace(strRequestLink, " ", VbCrlf)
			strRequestLink = Replace(strRequestLink, ",", VbCrlf)
			strRequestLink = Replace(strRequestLink, ";", VbCrlf)
			rs0("Link") = strRequestLink
		else
			rs0("Link") = Null
		end if
...
if lngIDPersonne <> empty then
	...
	if not rs0.EOF then
		Response.Write "<form action=persPage.asp method=post id=form1 name=form1>"
		...
		Response.Write "<tr><td>Email [*]</td><td><input type=text name=Email size=40 value=""" & rs0("Email") & """ class=box></td></tr>"
		Response.Write "<tr><td align=right><input type=submit value=Update class=box></td>"
		Response.Write "<td><input type=submit name=Del value=Delete class=box></td></tr>"
		Response.Write "<tr><td colspan=2>Biography<br><textarea name=Comm cols=80 rows=5 class=box>" & rs0("Comm") & "</textarea></td></tr>"
		Response.Write "<tr><td colspan=2><table cellpadding=0 cellspacing=0 border=0><tr valign=top>"
		Response.Write "<td>Links<br><textarea name=Link cols=40 rows=3 class=box>" & rs0("Link") & "</textarea></td>"
		Response.Write "<td><br>This person is """ & strUpload & "=" & rs0("IDPersonne") & """<br>use this code in other S-O-F sites to link this person.<br>(one link per line)</td>"
	...
	Response.Write "<form action=persPage.asp method=post>"
	...
	Response.Write "<tr><td>Email [*]</td><td><input type=text size=40 name=Email class=box></td></tr>"
	Response.Write "<tr><td align=center><input type=submit value=Add class=box></td><td>&nbsp;</td></tr>"
	Response.Write "<tr><td colspan=2>Biography<br><textarea cols=80 rows=5 name=Comm class=box></textarea></td></tr>"
	Response.Write "</table></form>"
end if
