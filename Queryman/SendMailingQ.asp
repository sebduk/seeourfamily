...
<Form Action=Send1.asp target=answer method=post>
		<td>
			...
			<textarea cols=30 rows=5 name=MyQuery>SELECT Email, FirstName, LastName, KeyRemove
FROM MailingLists
WHERE IsOnline AND ListUpdates</textarea><br>Query (List : Email, FirstName, LastName, KeyRemove)
		</td>
		<td>
			Title : <input type=text size=70 name=MyTitle value="Mailing List"><br>
			<textarea cols=70 rows=5 name=MyText>Dear [FirstName] [LastName],

Your Text (do not remove the footer)

----------
If you do not wish to receive our information anymore you can click this link.
http://dsm.iea.org/contacts/MailingLists/remove.asp?Email=[Email]&Key=[KeyRemove]
(If you cannot click on the link please copy/paste it in the address bar of you browser)

If you simply want to update your information you can click this link.
http://dsm.iea.org/contacts/MailingLists/reg.asp?Email=[Email]&From=OUT
----------
</textarea><br>Text (include FirstName, LastName between [])
		</td>
	</tr>
	<tr>
